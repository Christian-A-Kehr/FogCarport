...
class Calculate implements LogicInterface {
    ...
    public double TotalLengthRaftersWithSlope(Carport carport) {
        double lengthSingleRafterWithSlope = ((0.5 * carport.getWidth()) / Math.cos(carport.getRoof().getAngle())); // Der ganges med 0,5 for at kunne danne en retvinklet trekant. 
    ...
    public double beamsPrice(Carport carport) {
        ...
        double totalPrice = (lenghtPrBeam * BeamsNeeded(carport)) * carport.getRoof().getBeam().getMprice();
        return totalPrice; 
    }
